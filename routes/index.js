...
const ur_search = "http://www.omdbapi.com/?s=";
const ur_find = "http://www.omdbapi.com/?t=";
                                    ...
                                    newUser.save(function (err) {
                                        ...
                                        userModel.findOne({ username: registerUser.username, email: registerUser.email }, function (err, user) {
                                            ...
                                            if (user) {
                                                try {
                                                    const output = `
                                                    <p>MYMOV support team</p>
                                                    <br>
                                                    <h3>Hi ${user.username},</h3>
                                                    <br>
                                                    <p>This email sent you because of your registration in MYMOV</p>
                                                    <br>
                                                    <p>For activate your account click on the below link</p>
                                                    <br>
                                                    ...
                                                    let mailOptions = {
                                                        from: '"MYMOV web application" <mkoohaki.online@meisamkoohaki.online>', // sender address
                                                        to: registerUser.email, // email of receiver
                                                        subject: 'Activation account on MYMOV', // Subject line
...
router.post('/search', isAuthenticated, function (req, res) {    
    ...
    if (name) {
        ...
        request({
            url: ur_search+name+"&apikey=2bf3702a",
            ...
            try {
                var movies = [];
                const lines1 = body.substring(body.indexOf('[')+1);
                const lines2 = lines1.substring(0, lines1.indexOf(']'));
                const lines = lines2.split(',{');
                var n = 0;
                lines.forEach((line) => {
                    if(n==0)
                        line = line.substring(1);

                    var title_position = line.indexOf('"Title":"') + 9;
                    var title_body = line.substring(title_position);
                    var name = line.substring(title_position, title_position + title_body.indexOf('"'));
                    var year_position = line.indexOf('"Year":"') + 8;
                    var year_body = line.substring(year_position);
                    var year = line.substring(year_position, year_position + year_body.indexOf('"'));
                    var poster_position = line.indexOf('"Poster":"') + 10;
                    var poster_body = line.substring(poster_position);
                    var poster = line.substring(poster_position, poster_position + poster_body.indexOf('"'));

                    movies[n] = [name, year, poster];
                    n++;
                });
                ...
                itemModel.find({ userId: req.user.id }, function (err, items) {
                    ...
                    res.render("main", { user: req.user, items: items, name: name, movies: movies, title: 'Main' });
...
/* POST save in main page - It uses authenticat checking to avoid posting withought logging in */
router.post('/save/:name', isAuthenticated, function (req, res) {

    var name = req.params.name;
    var userId = req.body.userId;
    var userGoogle = req.body.seller;
    userModel.findById({ _id: userId }, function (err, user) {
        if (err)
            console.log(err);
        if (user) {

            itemModel.find({ userId: user._id }, function (err, movie) {
                if (err)
                    console.log(err);
                if (movie.length < 5) {
                    itemModel.find({ userId: user._id, name: name }, function (err, movie) {
                        if (err)
                            console.log(err);
                        if (!movie.length) {
                            try {
                                ...
                                request({
                                    url: ur_find + name + "&apikey=2bf3702a",
                                }, function (error, response, body) {
                                    ...
                                    try {
                                        ...
                                        var title_position = body.indexOf('"Title":"') + 9;
                                        var title_body = body.substring(title_position);
                                        var name = body.substring(title_position, title_position + title_body.indexOf('"'));
                                        var year_position = body.indexOf('"Year":"') + 8;
                                        var year_body = body.substring(year_position);
                                        var year = body.substring(year_position, year_position + year_body.indexOf('"'));
                                        var director_position = body.indexOf('"Director":"') + 12;
                                        var director_body = body.substring(director_position);
                                        var director = body.substring(director_position, director_position + director_body.indexOf('"'));
                                        var runtime_position = body.indexOf('"Runtime":"') + 11;
                                        var runtime_body = body.substring(runtime_position);
                                        var runtime = body.substring(runtime_position, runtime_position + runtime_body.indexOf('"'));
                                        var poster_position = body.indexOf('"Poster":"') + 10;
                                        var poster_body = body.substring(poster_position);
                                        var poster = body.substring(poster_position, poster_position + poster_body.indexOf('"'));

                                        var movie = { "seller": req.body.seller, "name": name, "director": director, "year": year, "runtime": runtime, "image": poster, "userId": req.body.userId };
                                        var item = new itemModel(movie);
                            ...
                            itemModel.find({ userId: req.user.id }, function (err, items) {
                                ...
                                res.render("main", { user: req.user, items: items, movies: movies, title: 'Main', message: 'Movie already is in the list!' });
            ...
            userGoogleModel.find({ username: userGoogle }, function (err, userGoogle) {
                if (err)
                    console.log(err);
                if (userGoogle.length) {

                    itemModel.find({ userId: req.user.id }, function (err, movie) {
                        if (err)
                            console.log(err);
                        if (movie.length < 5) {
                            itemModel.find({ userId: req.user.id, name: name }, function (err, movie) {
                                if (err)
                                    console.log(err);
                                if (!movie.length) {
                                    try {
...
/* POST delete in home page - It uses authenticat checking to avoid posting withought logging in */
...
router.get('/main', isAuthenticated, function (req, res, next) {
    ...
    userModel.find({}, function (err, users) {
        ...
        itemModel.find({ userId: req.user.id }, function (err, items) {
            ...
            if (user.active == true) {
                res.render("main", { user: req.user, items: items });
            }
        });
    });
});
        ...
        if (user) {
            if (bcrypt.compare(passwordTry, user.password, function (err, isMatch) {
                if (isMatch) {
                    ...
                    <h3>Hi ${user.username},</h3>
                    <br>
                    <p>This email sent you because of your request for retrieving your username in MYMOV</p>
                    <br>
                    <ul>  
                    ...
                    let mailOptions = {
                        ...
                        subject: 'Retrieving Username on MYMOV', // Subject line
                        html: output // html body
                    };
...
router.post('/update_account', isAuthenticated, upload.single('image'), function Create(req, response) {
    ...
    if (req.body.username && req.body.email) {
        ...
        response.render('account', { message: 'Fill out all fields' });
    }
});

/* POST password page - It will bring user to password page */
router.post('/change_password_account', isAuthenticated, function (req, res) {
